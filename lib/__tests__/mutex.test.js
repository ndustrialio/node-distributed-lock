...
describe('Mutex', () => {
  ...
  describe('withLock', () => {
    test('returns the result of the callback on success lock', async () => {
      ...
      expect(queryInterfaceMocks.query).toHaveBeenCalledWith('lockTable', { transaction, firstResult: true });
      expect(queryInterfaceMocks.query).toHaveBeenCalledWith('removeStaleLocks', { transaction, firstResult: true });
